...
def batchnorm(Ylogits, is_test, iteration, offset, convolutional=False, name="batchnorm"):
    with tf.name_scope(name):
        ...
        tf.summary.histogram("mean", m)
        v = tf.cond(is_test, lambda: exp_moving_avg.average(variance), lambda: variance)
        tf.summary.histogram("variance", v)
...
with tf.name_scope("conv1"):
    ...
    Y1bn, update_ema1 = batchnorm(Y1l, tst, iter, B1, convolutional=True, name="BN1")
    Y1r = tf.nn.relu(Y1bn)
    tf.summary.histogram("activations", Y1r)
    Y1 = tf.nn.dropout(Y1r, pkeep_conv, compatible_convolutional_noise_shape(Y1r))
    tf.summary.histogram("dropout", Y1)
...
with tf.name_scope("conv2"):
    ...
    Y2bn, update_ema2 = batchnorm(Y2l, tst, iter, B2, convolutional=True, name="BN2")
    Y2r = tf.nn.relu(Y2bn)
    tf.summary.histogram("activations", Y2r)
    Y2 = tf.nn.dropout(Y2r, pkeep_conv, compatible_convolutional_noise_shape(Y2r))
    tf.summary.histogram("dropout", Y2)
...
with tf.name_scope("conv3"):
    ...
    Y3bn, update_ema3 = batchnorm(Y3l, tst, iter, B3, convolutional=True, name="BN3")
    Y3r = tf.nn.relu(Y3bn)
    tf.summary.histogram("activations", Y3r)
    Y3 = tf.nn.dropout(Y3r, pkeep_conv, compatible_convolutional_noise_shape(Y3r))
    tf.summary.histogram("dropout", Y3)

# reshape the output from the third convolution for the fully connected layer
YY = tf.reshape(Y3, shape=[-1, 7 * 7 * M]) #flattened
...
with tf.name_scope("fc1"):
    ...
    Y4bn, update_ema4 = batchnorm(Y4l, tst, iter, B4, name="BN4")
    Y4r = tf.nn.relu(Y4bn)
    tf.summary.histogram("fc1/relu", Y4r)
    Y4 = tf.nn.dropout(Y4r, pkeep)
    tf.summary.histogram("dropout", Y4)
